...
build-common: ## - execute build common tasks
	@ dotnet clean notification.scheduler.job.sln
	@ dotnet restore notification.scheduler.job.sln

.PHONY: build
build: build-common ## - build a debug assembly to the current platform (windows, linux or darwin(mac))
	@ dotnet build --no-restore notification.scheduler.job.sln
	@ ls -lah $(path)/src/notification.scheduler.job/bin/Debug/net6.0/

.PHONY: build-release
build-release: build-common ## - build a release assembly to the current platform (windows, linux or darwin(mac))
	@ dotnet build -c Release --no-restore notification.scheduler.job.sln
	@ ls -lah $(path)/src/notification.scheduler.job/bin/Release/net6.0/
...
scan-sec:
	...
	@ security-scan notification.scheduler.job.sln --export=notification.sarif
	@ echo ""
...
publish: ## - build a release assembly to the current platform (windows, linux or darwin(mac))
	@ rm -rf $(path)/src/notification.scheduler.job/publish
	@ dotnet publish -c Release $(path)/src/notification.scheduler.job/notification.scheduler.job.csproj -o $(path)/src/notification.scheduler.job/publish
	@ rm -f $(path)/src/notification.scheduler.job/publish/*.pdb $(path)/src/notification.scheduler.job/publish/appsettings.Development.json
	@ ls -lah $(path)/src/notification.scheduler.job/publish

.PHONY: docker-build
docker-build: ## - build docker image
	@ cd $(path)/src/notification.scheduler.job && docker build -f Dockerfile -t $(imageName):$(version) .
...
docker-scan: ## - Scan for known vulnerabilities
	...
	@ cd $(path)/src/notification.scheduler.job &&  docker scan -f Dockerfile notification-scheduler
